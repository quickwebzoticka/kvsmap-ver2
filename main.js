...
$(document).ready(function(){
    ...
    $(document).on("click","[data-form-infim]", function () {
        var text = $(this).attr("data-form-infim");
        console.log(text);
        $("#FORM10_FIELD_form_id").val(text);
        console.log($("#FORM10_FIELD_form_id").val());
    });
...
function sliders(limiter){
    ...
    hodLinePercentage = $(document).find('.hod-top-line__percentage'),
    // colSlidesHod = $(document).find('.hod-slider-item').length,
    // hodLineWidthItem = 100/colSlidesHod,
    tecHodLineWidth = $('.hod-top-line').attr('data-info'),
    ...
    hodLinePercentage.text(tecHodLineWidth + '%');
    ...
    $.fancybox.defaults.afterShow = function(){
        let hotTooltipSliderItems =  $(document).find('.fancybox-container .hod-tooltip-left-item');
        // $('.hod-tooltip-left__slider').slick('setPosition');
        let hodTooltipSlider = $(document).find('.fancybox-container .hod-tooltip-left__slider').slick({
         arrows:false
        });
...
$(document).on('click', '.taxiopen', function(e){
    e.preventDefault();
    window.CTVWidget.sendMessageToFrame('show', {auto: false});
});
